...
void *philosopher(void *arg) {
    ...
    while (mealCount < max_meals) {
        ...
        if (chopstick[sub] == 1) { /* the left is available */
            ...
            if (chopstick[(sub + 1) % thread_num] == 1) { /* the right is available */
        ...
        else { /* the left chopstick is unavailable */
            printf("philosopher %d: left=%d\n", sub, chopstick[sub]);
